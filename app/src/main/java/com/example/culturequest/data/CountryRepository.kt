...
object CountryRepository {
    ...
    fun loadCountries(context: Context, onLoaded: (() -> Unit)? = null) {
        //CorotineScope - used to run code in the background
        //needed so the game doesn't freeze while loading
        CoroutineScope(Dispatchers.IO).launch {
            val jsonText = context.assets.open("countries_min.json").bufferedReader().use { it.readText() }
    ...
    fun getCountryByName(name: String): Country? {
        return countries?.find { it.name.common.equals(name, ignoreCase = true) }
    }
